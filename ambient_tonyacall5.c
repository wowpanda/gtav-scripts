...
void __EntryFunction__()
{
	...
	if (_GET_NUMBER_OF_INSTANCES_OF_SCRIPT_WITH_NAME_HASH(joaat("ambient_tonyacall5")) > 1)
	{
...
int func_1()
{
	...
	if ((VDIST2(vVar0, vVar3) > 10000f || IS_ENTITY_DEAD(uLocal_38, 0)) || func_21())
	{
		return 1;
	}
	switch (iLocal_40)
		...
		case 0:
			if (DOES_ENTITY_EXIST(uLocal_38) && !IS_ENTITY_DEAD(uLocal_38, 0))
			{
				if (IS_ENTITY_AT_COORD(uLocal_38, vLocal_57, 1f, 1f, 1f, 0, 1, 0))
				{
					...
					TASK_PERFORM_SEQUENCE(uLocal_38, uVar7);
					CLEAR_SEQUENCE_TASK(&uVar7);
					iLocal_40 = 1;
				}
			}
			...
			if (DOES_ENTITY_EXIST(uLocal_38) && !IS_ENTITY_DEAD(uLocal_38, 0))
			{
				if (GET_SEQUENCE_PROGRESS(uLocal_38) == 1)
				{
					if (!DOES_ENTITY_EXIST(uLocal_39))
					{
						...
						{
							fVar8 = GET_ENTITY_ANIM_CURRENT_TIME(uLocal_38, "amb@world_human_stand_mobile@female@standing@call@enter", "enter");
							if (fVar8 >= 0.157f)
							{
				...
				{
					iVar6 = (GET_RANDOM_INT_IN_RANGE(0, 65535) % 2);
					if (iVar6 == 0)
					{
