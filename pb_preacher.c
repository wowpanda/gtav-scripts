...
void func_1()
{
	if (TIMERB() > 7000)
	{
		...
		{
			if (IS_ENTITY_AT_ENTITY(PLAYER_PED_ID(), uLocal_52, 10f, 10f, 10f, 0, 1, 0) && iLocal_54 == 0)
			{
				SETTIMERB(0);
				vLocal_48 = { GET_ENTITY_COORDS(PLAYER_PED_ID(), 1) };
				OPEN_SEQUENCE_TASK(&uLocal_55);
				TASK_TURN_PED_TO_FACE_COORD(0, vLocal_48, 0);
				TASK_PLAY_ANIM(0, "amb@PREACHER", "PREACH", 8f, -8f, -1, 1, 0, 0, 0, 0);
				CLOSE_SEQUENCE_TASK(uLocal_55);
				if (!IS_ENTITY_DEAD(uLocal_52, 0))
				{
					TASK_PERFORM_SEQUENCE(uLocal_52, uLocal_55);
				}
				CLEAR_SEQUENCE_TASK(&uLocal_55);
				iLocal_54 = 1;
			}
			if (IS_ENTITY_AT_ENTITY(PLAYER_PED_ID(), uLocal_52, 3f, 3f, 3f, 0, 1, 0) && iLocal_53 == 0)
			{
				vLocal_48 = { GET_ENTITY_COORDS(PLAYER_PED_ID(), 1) };
				SETTIMERB(0);
				OPEN_SEQUENCE_TASK(&uLocal_55);
...
void func_2()
{
	...
	uLocal_52 = CREATE_PED(19, joaat("s_m_m_strpreach_01"), vLocal_45.x, vLocal_45.y, vLocal_48.z, 0f, 1, 1);
	if (!IS_ENTITY_DEAD(uLocal_52, 0))
	{
		OPEN_SEQUENCE_TASK(&uLocal_55);
		TASK_TURN_PED_TO_FACE_COORD(0, vLocal_48, 0);
