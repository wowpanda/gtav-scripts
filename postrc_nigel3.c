...
void func_1()
{
	...
	{
		case 0:
			if ((GET_GAME_TIMER() - iLocal_187) > 5000 && HAS_ENTITY_BEEN_DAMAGED_BY_ENTITY(uLocal_188, PLAYER_PED_ID(), 1))
			{
				if (func_3(&uLocal_22, "NIGE3AU", "NIGEL3_REACT", 7, 0, 0, 0))
				{
					CLEAR_ENTITY_LAST_DAMAGE_ENTITY(uLocal_188);
					iLocal_21 = 1;
				}
			}
			else if ((GET_GAME_TIMER() - iLocal_187) > 12000 && VDIST2(GET_ENTITY_COORDS(PLAYER_PED_ID(), 1), GET_ENTITY_COORDS(uLocal_188, 1)) <= 900f)
			{
				if (func_3(&uLocal_22, "NIGE3AU", "NIGEL3_REACT", 7, 0, 0, 0))
				{
					iLocal_21 = 1;
...
{
	iLocal_187 = GET_GAME_TIMER();
	func_16(&uLocal_22, 2, PLAYER_PED_ID(), "TREVOR", 0, 1);
	func_16(&uLocal_22, 5, uLocal_188, "DINAPOLI", 0, 1);
	iLocal_20 = 1;
}
...
int func_17()
{
	...
	fVar0 = 200f;
	if (VDIST2(GET_ENTITY_COORDS(PLAYER_PED_ID(), 1), GET_ENTITY_COORDS(uLocal_188, 1)) >= (fVar0 * fVar0) && !IS_ENTITY_ON_SCREEN(uLocal_188))
	{
		return 0;
	}
	return 1;
...
int func_19()
{
	if (DOES_ENTITY_EXIST(uLocal_188))
	{
		if (!IS_PED_INJURED(uLocal_188))
		{
			return 1;
		}
	}
...
int func_25()
{
	...
	{
		iVar17 = GET_PED_NEARBY_PEDS(PLAYER_PED_ID(), &uVar0, -1);
		iVar18 = 0;
		while (iVar18 <= (iVar17 - 1))
		{
			if (uVar0[iVar18] != 0 && DOES_ENTITY_EXIST(uVar0[iVar18]))
			{
				if (GET_ENTITY_MODEL(uVar0[iVar18]) == joaat("u_m_m_aldinapoli"))
				{
					SET_ENTITY_AS_MISSION_ENTITY(uVar0[iVar18], 1, 1);
...
void func_26()
{
	...
	if (DOES_ENTITY_EXIST(uLocal_188))
	{
		SET_PED_AS_NO_LONGER_NEEDED(&uLocal_188);
	}
