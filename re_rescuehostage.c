		...
		if (IS_WORLD_POINT_WITHIN_BRAIN_ACTIVATION_RANGE())
		{
			switch (iLocal_47)
			{
				...
				case 1:
					...
					else if (!IS_PED_INJURED(uLocal_54))
					{
						func_99();
						func_98();
					}
					...
					else
					{
						...
						{
							if (DOES_BLIP_EXIST(uLocal_57))
							{
								REMOVE_BLIP(&uLocal_57);
							}
						...
						if (!IS_PED_INJURED(uLocal_53))
						{
							if (bLocal_76)
							{
								if (IS_SYNCHRONIZED_SCENE_RUNNING(uLocal_67))
								{
									if (GET_SYNCHRONIZED_SCENE_PHASE(uLocal_67) == 1f)
									{
										if (!IS_PED_INJURED(uLocal_53))
										{
				...
				case 2:
					uLocal_56 = GET_RANDOM_VEHICLE_IN_SPHERE(GET_ENTITY_COORDS(PLAYER_PED_ID(), 1), 35f, 0, 101383);
					if (IS_VEHICLE_DRIVEABLE(uLocal_56, 0))
					{
						if (!bLocal_100)
						{
					...
					if (!iLocal_86)
					{
						if (CREATE_INCIDENT(7, vLocal_48, 2, 0f, &uLocal_88, 0, 0))
						{
							iLocal_86 = 1;
						}
					}
				...
				case 3:
					...
					else
					{
						...
						uLocal_56 = GET_RANDOM_VEHICLE_IN_SPHERE(vLocal_48, 5f, joaat("ambulance"), 0);
						if (IS_VEHICLE_DRIVEABLE(uLocal_56, 0))
						{
							SETTIMERA(0);
							iLocal_84 = 1;
						}
						uLocal_55 = GET_RANDOM_PED_AT_COORD(vLocal_48, 1.5f, 1.5f, 1.5f, -1);
						...
						{
							if (CREATE_INCIDENT(5, vLocal_48, 2, 0f, &uLocal_89, 0, 0))
							{
								iLocal_86 = 1;
							}
						}
...
{
	_PLAY_AMBIENT_SPEECH_WITH_VOICE(uParam0, sParam1, sParam2, func_2(iParam3), 0);
}
...
void func_3()
{
	if (func_4())
	{
		if (!IS_PED_INJURED(uLocal_53))
		{
			...
			TASK_SMART_FLEE_COORD(uLocal_53, vLocal_48, 50f, -1, 0, 0);
			SET_PED_KEEP_TASK(uLocal_53, 1);
			SET_PED_AS_NO_LONGER_NEEDED(&uLocal_53);
			func_48();
		}
		if (!IS_PED_INJURED(uLocal_54))
		{
			SET_BLOCKING_OF_NON_TEMPORARY_EVENTS(uLocal_54, 0);
			TASK_SMART_FLEE_COORD(uLocal_54, vLocal_48, 50f, -1, 0, 0);
			SET_PED_KEEP_TASK(uLocal_54, 1);
			SET_PED_AS_NO_LONGER_NEEDED(&uLocal_54);
			func_48();
		}
	}
	if (!IS_PED_INJURED(uLocal_53))
	{
		...
		{
			if (IS_PED_RAGDOLL(uLocal_53))
			{
				SET_ENTITY_HEALTH(uLocal_53, 0, 0);
			}
		}
		else if (IS_PED_RAGDOLL(uLocal_53))
		{
			SET_BLOCKING_OF_NON_TEMPORARY_EVENTS(uLocal_53, 0);
			TASK_SMART_FLEE_COORD(uLocal_53, vLocal_48, 50f, -1, 0, 0);
			SET_PED_KEEP_TASK(uLocal_53, 1);
	...
	if (!IS_PED_INJURED(uLocal_54))
	{
		if (HAS_ENTITY_BEEN_DAMAGED_BY_ENTITY(uLocal_54, PLAYER_PED_ID(), 1))
		{
			...
			{
				SET_ENTITY_HEALTH(uLocal_54, 0, 0);
			}
		}
		else if (IS_PED_RAGDOLL(uLocal_54))
		{
			SET_BLOCKING_OF_NON_TEMPORARY_EVENTS(uLocal_54, 0);
			TASK_SMART_FLEE_COORD(uLocal_54, vLocal_48, 50f, -1, 0, 0);
			SET_PED_KEEP_TASK(uLocal_54, 1);
	...
	else if (!IS_PED_INJURED(uLocal_53))
	{
		SET_BLOCKING_OF_NON_TEMPORARY_EVENTS(uLocal_53, 0);
		TASK_SMART_FLEE_COORD(uLocal_53, vLocal_48, 50f, -1, 0, 0);
		SET_PED_KEEP_TASK(uLocal_53, 1);
...
bool func_4()
{
	...
	if (!IS_PED_INJURED(uLocal_53))
	{
		if (IS_BULLET_IN_AREA(GET_PED_BONE_COORDS(uLocal_53, 31086, 0f, 0f, 0f), 15f, 1))
		{
			bVar0 = true;
		}
		vVar1 = { vVar1 + GET_PED_BONE_COORDS(uLocal_53, 31086, 0f, 0f, 0f) };
		vVar4 = { vVar4 + GET_PED_BONE_COORDS(uLocal_53, 31086, 0f, 0f, 0f) };
		...
		if (IS_EXPLOSION_IN_ANGLED_AREA(-1, -90.68654f, 6411.883f, 36.00729f, -103.86f, 6398.848f, 30.19453f, 23f) || IS_EXPLOSION_IN_SPHERE(-1, GET_PED_BONE_COORDS(uLocal_53, 31086, 0f, 0f, 0f), 20f))
		{
			bLocal_98 = true;
			bVar0 = true;
		}
		if (_0x2F09F7976C512404(GET_PED_BONE_COORDS(uLocal_53, 31086, 0f, 0f, 0f), 6f))
		{
			bVar0 = true;
			bLocal_98 = true;
		}
...
void func_29()
{
	...
	{
		func_31(13, FLOOR(Global_106565.f_10188.f_3853));
	}
...
int func_47()
{
	if (!iLocal_81)
	{
		...
		if (!IS_PED_INJURED(uLocal_53))
		{
			if (iLocal_68 == -1)
			{
				SET_PED_CAN_RAGDOLL(uLocal_53, 0);
				SET_DISPATCH_TIME_BETWEEN_SPAWN_ATTEMPTS_MULTIPLIER(5, 0.5f);
				SET_DISPATCH_TIME_BETWEEN_SPAWN_ATTEMPTS(5, 1f);
				SET_PED_GENERATES_DEAD_BODY_EVENTS(uLocal_53, 1);
				SET_PED_SUFFERS_CRITICAL_HITS(uLocal_53, 1);
				TASK_PLAY_ANIM(uLocal_53, sLocal_267, "girl_villian_shot", 8f, -2f, -1, 10, 0, 0, 0, 0);
				iLocal_68 = CREATE_SYNCHRONIZED_SCENE(vLocal_70, vLocal_73, 2);
				if (!IS_PED_INJURED(uLocal_54))
				{
					SET_PED_RELATIONSHIP_GROUP_HASH(uLocal_54, uLocal_59);
					if (!IS_ENTITY_IN_ANGLED_AREA(PLAYER_PED_ID(), -91.30704f, 6391.854f, 30.6403f, -102.5595f, 6403.103f, 32.45444f, 8.5f, 0, 1, 0))
					{
						TASK_SYNCHRONIZED_SCENE(uLocal_54, iLocal_68, sLocal_267, "bystander_villian_shot", 1000f, -2f, 5, 0, 1148846080, 0);
					}
					else
					{
						SET_BLOCKING_OF_NON_TEMPORARY_EVENTS(uLocal_54, 0);
				...
				TASK_SYNCHRONIZED_SCENE(uLocal_53, iLocal_68, sLocal_267, "girl_villian_shot", 1000f, -2f, 5, 0, 1148846080, 0);
			}
			else if (IS_SYNCHRONIZED_SCENE_RUNNING(iLocal_68))
			{
				...
				if (GET_SYNCHRONIZED_SCENE_PHASE(iLocal_68) == 1f)
				{
					iLocal_68 = CREATE_SYNCHRONIZED_SCENE(vLocal_70, vLocal_73, 2);
					func_1(uLocal_53, "WHIMPER", "WAVELOAD_PAIN_FEMALE", 24);
					SET_SYNCHRONIZED_SCENE_LOOPED(iLocal_68, 1);
					if (!IS_PED_INJURED(uLocal_54))
					{
						if (bLocal_87)
						{
							TASK_SYNCHRONIZED_SCENE(uLocal_54, iLocal_68, sLocal_267, "bystander_helping_girl_loop", 1000f, -2f, 5, 0, 1148846080, 0);
						}
						else
						{
							OPEN_SEQUENCE_TASK(&uLocal_79);
							TASK_LOOK_AT_ENTITY(0, uLocal_53, -1, 1024, 2);
							TASK_GO_TO_ENTITY(0, uLocal_53, 20000, 3f, 1f, 1073741824, 0);
							TASK_STAND_STILL(0, -1);
							CLOSE_SEQUENCE_TASK(uLocal_79);
							TASK_PERFORM_SEQUENCE(uLocal_54, uLocal_79);
						}
						SET_PED_KEEP_TASK(uLocal_54, 1);
						SET_PED_RELATIONSHIP_GROUP_HASH(uLocal_54, uLocal_61);
					}
					TASK_SYNCHRONIZED_SCENE(uLocal_53, iLocal_68, sLocal_267, "girl_helping_girl_loop", 1000f, -2f, 5, 0, 1148846080, 0);
					SET_PED_KEEP_TASK(uLocal_53, 1);
					SET_PED_RELATIONSHIP_GROUP_HASH(uLocal_53, uLocal_61);
					SET_PED_CAN_RAGDOLL(uLocal_53, 1);
...
void func_51()
{
	if (((((((((((IS_ENTITY_IN_ANGLED_AREA(PLAYER_PED_ID(), -91.30704f, 6391.854f, 30.6403f, -102.5595f, 6403.103f, 32.45444f, 8.5f, 0, 1, 0) && _CAN_PED_SEE_PED(uLocal_52, PLAYER_PED_ID())) || (func_61(uLocal_52, &uLocal_62, &iLocal_63, 0, 0, 1077936128, 0) && _CAN_PED_SEE_PED(uLocal_52, PLAYER_PED_ID()))) || (func_60(1) && _CAN_PED_SEE_PED(uLocal_52, PLAYER_PED_ID()))) || func_58()) || func_57()) || func_4()) || func_56()) || IS_PED_RAGDOLL(uLocal_52)) || func_55()) || func_54()) || func_52(uLocal_52))
	{
		if (!IS_PED_PERFORMING_STEALTH_KILL(PLAYER_PED_ID()))
		{
			if (!HAS_PED_BEEN_DAMAGED_BY_WEAPON(uLocal_52, joaat("weapon_stungun"), 0))
			{
				if (!IS_PED_INJURED(uLocal_53))
				{
					if (!IS_PED_RAGDOLL(uLocal_52))
					{
						uLocal_67 = CREATE_SYNCHRONIZED_SCENE(vLocal_70, vLocal_73, 2);
						SET_SYNCHRONIZED_SCENE_LOOPED(uLocal_67, 0);
						TASK_SYNCHRONIZED_SCENE(uLocal_53, uLocal_67, sLocal_267, "girl_girl_shot", 4f, -4f, 5, 0, 1148846080, 0);
						TASK_SYNCHRONIZED_SCENE(uLocal_52, uLocal_67, sLocal_267, "villian_girl_shot", 4f, -4f, 5, 0, 1148846080, 0);
						SET_PED_KEEP_TASK(uLocal_53, 1);
						SET_PED_CAN_RAGDOLL(uLocal_53, 0);
					}
				}
				if (!IS_PED_INJURED(uLocal_54))
				{
					SET_PED_CAN_RAGDOLL(uLocal_54, 1);
				}
				bLocal_76 = true;
			}
			else
			{
				SET_ENTITY_HEALTH(uLocal_52, 50, 0);
			}
		}
	}
	else if (TIMERA() > 6000)
	{
		if ((IS_PLAYER_FREE_AIMING_AT_ENTITY(PLAYER_ID(), uLocal_52) && _CAN_PED_SEE_PED(uLocal_52, PLAYER_PED_ID())) || (IS_PLAYER_TARGETTING_ENTITY(PLAYER_ID(), uLocal_52) && _CAN_PED_SEE_PED(uLocal_52, PLAYER_PED_ID())))
		{
			if (DOES_BLIP_EXIST(uLocal_57))
			{
				_0x75A16C3DA34F1245(uLocal_57, 1);
			}
			if (!iLocal_80)
			{
				if (!IS_ENTITY_IN_ANGLED_AREA(PLAYER_PED_ID(), -91.30704f, 6391.854f, 30.6403f, -102.5595f, 6403.103f, 32.45444f, 8.5f, 0, 1, 0))
...
int func_52(var uParam0)
{
	...
	if (HAS_PED_GOT_WEAPON_COMPONENT(PLAYER_PED_ID(), uVar0, joaat("component_at_ar_flsh")))
	{
		if (IS_PED_WEAPON_COMPONENT_ACTIVE(PLAYER_PED_ID(), uVar0, joaat("component_at_ar_flsh")))
		{
			bVar1 = true;
		}
	}
	else if (HAS_PED_GOT_WEAPON_COMPONENT(PLAYER_PED_ID(), uVar0, joaat("component_at_pi_flsh")))
	{
		if (IS_PED_WEAPON_COMPONENT_ACTIVE(PLAYER_PED_ID(), uVar0, joaat("component_at_pi_flsh")))
		{
			bVar1 = true;
		}
	}
	if (bVar1)
	{
		...
		{
			if (VDIST(GET_ENTITY_COORDS(uParam0, 1), func_53(PLAYER_ID())) < 8f)
			{
				return 1;
			}
		}
...
int func_55()
{
	...
	fVar0 = 75f;
	uVar36 = GET_RANDOM_PED_AT_COORD(GET_ENTITY_COORDS(uLocal_52, 0), fVar0, fVar0, fVar0, -1);
	if (!IS_PED_INJURED(uVar36))
	{
		if (GET_ENTITY_MODEL(uVar36) == joaat("s_f_y_cop_01") || GET_ENTITY_MODEL(uVar36) == joaat("s_m_y_cop_01"))
		{
			return 1;
		}
	}
	uVar37 = GET_RANDOM_VEHICLE_IN_SPHERE(GET_ENTITY_COORDS(uLocal_52, 0), fVar0, 0, 66561);
	if (IS_VEHICLE_DRIVEABLE(uVar37, 0))
	{
		return 1;
	}
	iVar35 = GET_PED_NEARBY_VEHICLES(uLocal_52, &uVar1);
	if (iVar35 > 0)
	{
		iVar34 = 0;
		while (iVar34 < iVar35)
...
int func_56()
{
	if (IS_ENTITY_AT_ENTITY(uLocal_52, PLAYER_PED_ID(), 2f, 2f, 1.5f, 0, 1, 0))
	{
		if (CAN_PED_HEAR_PLAYER(PLAYER_ID(), uLocal_52))
		{
			return 1;
		}
	}
...
bool func_57()
{
	...
	if (IS_PED_IN_ANY_VEHICLE(PLAYER_PED_ID(), 0))
	{
		if (GET_ENTITY_MODEL(GET_VEHICLE_PED_IS_IN(PLAYER_PED_ID(), 0)) == joaat("bulldozer"))
		{
			...
			if (!IS_PED_INJURED(uLocal_53))
			{
				if (IS_ENTITY_TOUCHING_ENTITY(GET_VEHICLE_PED_IS_IN(PLAYER_PED_ID(), 0), uLocal_53))
				{
					bVar0 = true;
				}
			}
			if (!IS_PED_INJURED(uLocal_52))
			{
				if (IS_ENTITY_TOUCHING_ENTITY(GET_VEHICLE_PED_IS_IN(PLAYER_PED_ID(), 0), uLocal_52))
				{
					bVar0 = true;
				}
			}
...
bool func_58()
{
	...
	if (IS_PED_IN_ANY_VEHICLE(PLAYER_PED_ID(), 0))
	{
		...
		{
			if (GET_VEHICLE_TRAILER_VEHICLE(GET_VEHICLE_PED_IS_IN(PLAYER_PED_ID(), 0), &uVar14))
			{
				GET_MODEL_DIMENSIONS(GET_ENTITY_MODEL(uVar14), &uVar5, &Var8);
		...
		{
			if (IS_ENTITY_AT_ENTITY(PLAYER_PED_ID(), uLocal_52, (fVar12 + 17f), (fVar12 + 17f), 6f, 0, 1, 0))
			{
				bVar1 = true;
			}
		}
		...
		{
			if (IS_ENTITY_AT_ENTITY(PLAYER_PED_ID(), uLocal_52, (fVar12 + 12f), (fVar12 + 12f), 6f, 0, 1, 0))
			{
				bVar1 = true;
			}
		}
		...
		{
			if (IS_ENTITY_AT_ENTITY(PLAYER_PED_ID(), uLocal_52, (fVar12 + 8f), (fVar12 + 8f), 6f, 0, 1, 0))
			{
				bVar1 = true;
			}
		}
	...
	uVar13 = GET_PLAYERS_LAST_VEHICLE();
	if (DOES_ENTITY_EXIST(uVar13))
	{
		if (IS_ENTITY_IN_ANGLED_AREA(uVar13, -91.56341f, 6391.747f, 30.6397f, -101.2611f, 6401.7f, 32.45449f, 7.5f, 0, 1, 0))
		{
			bVar0 = true;
		}
	}
...
void func_68()
{
	if (((IS_ENTITY_IN_ANGLED_AREA(PLAYER_PED_ID(), -62.6571f, 6447.08f, 40.49928f, -153.6411f, 6354.579f, 23.99063f, 144.25f, 0, 1, 0) || IS_SPHERE_VISIBLE(vLocal_48, 15f)) || bLocal_76) || func_4())
	{
		uLocal_57 = func_79(uLocal_52, 1, 0);
		if (DOES_BLIP_EXIST(uLocal_57))
		{
			_0x75A16C3DA34F1245(uLocal_57, 0);
		}
		func_69(1);
		iLocal_99 = GET_GAME_TIMER();
...
int func_73()
{
	switch (func_74(&Global_25824, 0, 5, 0, GET_ID_OF_THIS_THREAD()))
	{
		case 1:
			return 1;
			break;
...
int func_81(var uParam0, bool bParam1, bool bParam2)
{
	...
	{
		SET_BLIP_SCALE(uVar0, func_82(NETWORK_IS_GAME_IN_PROGRESS(), 1f, 1f));
		if (!bParam2)
		{
	...
	{
		SET_BLIP_SCALE(uVar0, func_82(NETWORK_IS_GAME_IN_PROGRESS(), 0.7f, 0.7f));
		SET_BLIP_AS_FRIENDLY(uVar0, bParam1);
	}
...
int func_95()
{
	if ((Global_106554 == func_44() && GET_RANDOM_EVENT_FLAG()) && Global_106555)
	{
		return 1;
	}
	return 0;
...
void func_96()
{
	...
	if (!IS_PED_INJURED(uLocal_52))
	{
		SET_PED_RESET_FLAG(uLocal_52, 156, 1);
		if (IS_SYNCHRONIZED_SCENE_RUNNING(uLocal_67))
		{
			if (GET_SYNCHRONIZED_SCENE_PHASE(uLocal_67) > 0.1f)
			{
				if (!iLocal_82)
				{
					if (!IS_PED_INJURED(uLocal_53))
					{
						SET_PED_SHOOTS_AT_COORD(uLocal_52, GET_PED_BONE_COORDS(uLocal_53, 31086, 0f, 0f, -0.1f), 0);
					}
					iLocal_82 = 1;
					iLocal_80 = 1;
					func_97();
		...
		if (IS_SYNCHRONIZED_SCENE_RUNNING(uLocal_67))
		{
			if (GET_SYNCHRONIZED_SCENE_PHASE(uLocal_67) > 0.6f)
			{
				func_97();
			}
		}
...
void func_97()
{
	if (!iLocal_83)
	{
		if (!IS_PED_INJURED(uLocal_52))
		{
			CLEAR_PED_TASKS(uLocal_52);
			SET_RELATIONSHIP_BETWEEN_GROUPS(5, uLocal_60, uLocal_59);
			SET_RELATIONSHIP_BETWEEN_GROUPS(5, uLocal_59, uLocal_60);
			SET_RELATIONSHIP_BETWEEN_GROUPS(0, uLocal_61, uLocal_59);
			SET_RELATIONSHIP_BETWEEN_GROUPS(2, uLocal_61, uLocal_60);
			SET_RELATIONSHIP_BETWEEN_GROUPS(2, uLocal_60, uLocal_61);
			if (IS_ENTITY_AT_ENTITY(uLocal_52, PLAYER_PED_ID(), 50f, 50f, 50f, 0, 1, 0))
			{
				TASK_COMBAT_PED(uLocal_52, PLAYER_PED_ID(), 0, 16);
				SET_PED_TARGET_LOSS_RESPONSE(uLocal_52, 1);
			}
			else
			{
				TASK_SMART_FLEE_PED(uLocal_52, PLAYER_PED_ID(), 200f, -1, 0, 0);
			}
			SET_PED_KEEP_TASK(uLocal_52, 1);
...
void func_99()
{
	...
	func_102();
	GET_CLOSEST_VEHICLE_NODE(GET_ENTITY_COORDS(PLAYER_PED_ID(), 1), &vLocal_48, 1, 1077936128, 0);
	...
	if (iLocal_80)
	{
		if (!HAS_ENTITY_BEEN_DAMAGED_BY_WEAPON(uLocal_54, joaat("weapon_pistol"), 0) && IS_PED_INJURED(uLocal_52))
		{
			if (!bLocal_100)
			{
				iLocal_47 = 2;
		...
		else
		{
			if (!IS_PED_INJURED(uLocal_54))
			{
				if (!IS_PED_INJURED(uLocal_52))
				{
					TASK_SMART_FLEE_PED(uLocal_54, uLocal_52, 100f, -1, 0, 0);
				}
				else
				{
					TASK_SMART_FLEE_PED(uLocal_54, PLAYER_PED_ID(), 100f, -1, 0, 0);
				}
				SET_PED_KEEP_TASK(uLocal_54, 1);
				if (!bLocal_100)
				{
...
void func_102()
{
	...
	if (bLocal_98 == 0)
	{
		if (!IS_PED_INJURED(uLocal_54) && !IS_PED_INJURED(PLAYER_PED_ID()))
		{
			if (iLocal_80)
			{
				OPEN_SEQUENCE_TASK(&uLocal_79);
				TASK_GO_TO_ENTITY(0, PLAYER_PED_ID(), 20000, 5f, 1073741824, 1073741824, 0);
				TASK_LOOK_AT_ENTITY(0, PLAYER_PED_ID(), -1, 2048, 2);
				TASK_PLAY_ANIM(0, sLocal_267, "bystander_bystander_angry", 8f, -8f, 1500, 0, 0, 0, 0, 0);
				TASK_SMART_FLEE_PED(0, PLAYER_PED_ID(), 150f, -1, 0, 0);
				CLOSE_SEQUENCE_TASK(uLocal_79);
				TASK_PERFORM_SEQUENCE(uLocal_54, uLocal_79);
				CLEAR_SEQUENCE_TASK(&uLocal_79);
				SET_PED_KEEP_TASK(uLocal_54, 1);
			}
			else
			{
				OPEN_SEQUENCE_TASK(&uLocal_79);
				TASK_GO_STRAIGHT_TO_COORD(0, vLocal_48, 1f, 20000, 1193033728, 1056964608);
				TASK_LOOK_AT_COORD(0, vLocal_48, -1, 2048, 2);
				TASK_PLAY_ANIM(0, sLocal_267, "bystander_bystander_angry", 8f, -8f, 1500, 0, 0, 0, 0, 0);
				CLOSE_SEQUENCE_TASK(uLocal_79);
				TASK_PERFORM_SEQUENCE(uLocal_54, uLocal_79);
				CLEAR_SEQUENCE_TASK(&uLocal_79);
...
void func_103()
{
	...
	else if (!IS_AMBIENT_SPEECH_PLAYING(uLocal_53))
	{
		PLAY_PAIN(uLocal_53, 4, 0, 0);
	}
}

void func_104()
{
	if (IS_ENTITY_AT_COORD(PLAYER_PED_ID(), vLocal_48, 50f, 50f, 25f, 0, 1, 0))
	{
		iLocal_266 = 0;
	}
	else
...
int func_106()
{
	...
	{
		CLEAR_AREA(vLocal_48, 5f, 1, 1, 0, 0);
		uLocal_53 = CREATE_PED(5, iVar2, vLocal_48, fLocal_51, 1, 1);
		SET_PED_SUFFERS_CRITICAL_HITS(uLocal_53, 1);
		SET_AMBIENT_VOICE_NAME(uLocal_53, sVar3);
		SET_PED_CONFIG_FLAG(uLocal_53, 20, 1);
		uLocal_52 = CREATE_PED(22, iVar0, vLocal_48, fLocal_51, 1, 1);
		SET_ENTITY_HEALTH(uLocal_52, 125, 0);
		SET_PED_MAX_HEALTH(uLocal_52, 125);
		SET_PED_FLEE_ATTRIBUTES(uLocal_52, 128, 1);
		SET_PED_SUFFERS_CRITICAL_HITS(uLocal_52, 1);
		SET_PED_CONFIG_FLAG(uLocal_52, 42, 1);
		SET_PED_HEARING_RANGE(uLocal_52, 1.5f);
		STOP_PED_SPEAKING(uLocal_52, 1);
		SET_PED_SEEING_RANGE(uLocal_52, 40f);
		SET_PED_CONFIG_FLAG(uLocal_52, 20, 1);
		SET_PED_VISUAL_FIELD_MIN_ANGLE(uLocal_52, fLocal_93);
		SET_PED_VISUAL_FIELD_MAX_ANGLE(uLocal_52, fLocal_94);
		SET_PED_VISUAL_FIELD_PERIPHERAL_RANGE(uLocal_52, fLocal_95);
		SET_PED_VISUAL_FIELD_CENTER_ANGLE(uLocal_52, fLocal_96);
		uLocal_54 = CREATE_PED(4, iVar1, -98.113f, 6405.354f, 30.6005f, fLocal_51, 1, 1);
		...
		uLocal_65 = ADD_SCENARIO_BLOCKING_AREA(Vector(31.64024f, 6405.058f, -98.41389f) - Vector(40f, 40f, 40f), Vector(31.64024f, 6405.058f, -98.41389f) + Vector(40f, 40f, 40f), 0, 1, 1, 1);
		SET_PED_CAN_BE_TARGETTED(uLocal_54, 0);
		STOP_PED_SPEAKING(uLocal_54, 1);
		vLocal_48.z = (vLocal_48.z + 1f);
		vLocal_70 = { vLocal_48 };
		vLocal_73 = { 0f, 0f, fLocal_51 };
		uLocal_67 = CREATE_SYNCHRONIZED_SCENE(vLocal_70, vLocal_73, 2);
		SET_SYNCHRONIZED_SCENE_LOOPED(uLocal_67, 1);
		TASK_SYNCHRONIZED_SCENE(uLocal_52, uLocal_67, sLocal_267, "villian_struggle_loop", 1000f, -4f, 5, 0, 1148846080, 0);
		TASK_SYNCHRONIZED_SCENE(uLocal_53, uLocal_67, sLocal_267, "girl_struggle_loop", 1000f, -4f, 5, 0, 1148846080, 0);
		uLocal_69 = CREATE_SYNCHRONIZED_SCENE(vLocal_70, vLocal_73, 2);
		SET_SYNCHRONIZED_SCENE_LOOPED(uLocal_69, 1);
		TASK_SYNCHRONIZED_SCENE(uLocal_54, uLocal_69, sLocal_267, "bystander_taking_cover", 1000f, -1.5f, 5, 0, 1148846080, 0);
		SET_PED_CONFIG_FLAG(uLocal_54, 185, 1);
		GIVE_WEAPON_TO_PED(uLocal_52, joaat("weapon_pistol"), -1, 1, 1);
		SET_PED_AS_ENEMY(uLocal_52, 1);
		ADD_RELATIONSHIP_GROUP("re_rescuehostage relManager", &uLocal_61);
		ADD_RELATIONSHIP_GROUP("re_rescuehostage relBadGuy", &uLocal_60);
		uLocal_59 = GET_PED_RELATIONSHIP_GROUP_HASH(PLAYER_PED_ID());
		SET_RELATIONSHIP_BETWEEN_GROUPS(5, uLocal_60, uLocal_59);
		SET_RELATIONSHIP_BETWEEN_GROUPS(5, uLocal_59, uLocal_60);
		SET_PED_RELATIONSHIP_GROUP_HASH(uLocal_54, uLocal_61);
		SET_PED_RELATIONSHIP_GROUP_HASH(uLocal_52, uLocal_60);
		SET_BLOCKING_OF_NON_TEMPORARY_EVENTS(uLocal_53, 1);
		SET_BLOCKING_OF_NON_TEMPORARY_EVENTS(uLocal_54, 1);
		SET_BLOCKING_OF_NON_TEMPORARY_EVENTS(uLocal_52, 1);
		SET_PED_COMBAT_ATTRIBUTES(uLocal_54, 11, 1);
		SET_PED_COMBAT_ATTRIBUTES(uLocal_54, 17, 1);
		SET_PED_COMBAT_ATTRIBUTES(uLocal_54, 13, 0);
		SET_PED_COMBAT_ATTRIBUTES(uLocal_54, 5, 0);
		SET_PED_PATH_CAN_USE_CLIMBOVERS(uLocal_54, 0);
		SET_PED_COMBAT_ATTRIBUTES(uLocal_53, 11, 1);
		SET_PED_PATH_CAN_USE_CLIMBOVERS(uLocal_53, 0);
		SET_PED_COMBAT_ATTRIBUTES(uLocal_53, 13, 0);
		SET_PED_COMBAT_ATTRIBUTES(uLocal_53, 5, 0);
		...
		if (DOES_ENTITY_EXIST(PLAYER_PED_ID()))
		{
			if (!IS_PED_INJURED(PLAYER_PED_ID()))
			{
				...
				switch (iParam0)
				{
					case 9:
					case 0:
						if (((((((((((((((((!IS_PLAYER_READY_FOR_CUTSCENE(PLAYER_ID()) || IS_ENTITY_IN_AIR(PLAYER_PED_ID())) || IS_PED_GETTING_INTO_A_VEHICLE(PLAYER_PED_ID())) || IS_PED_RAGDOLL(PLAYER_PED_ID())) || IS_PED_FALLING(PLAYER_PED_ID())) || IS_PLAYER_BEING_ARRESTED(PLAYER_ID(), 1)) || IS_PLAYER_CLIMBING(PLAYER_ID())) || IS_PED_IN_COMBAT(PLAYER_PED_ID(), 0)) || func_149()) || Global_105612) || Global_25767) || func_148()) || func_19(8, -1)) || func_147()) || func_146()) || func_145()) || func_144()) || Global_106565.f_7682.f_919[iVar0] == 5)
						{
							return 0;
						}
						break;
					...
					case 1:
						if (((((((((IS_PLAYER_BEING_ARRESTED(PLAYER_ID(), 1) || func_149()) || Global_25767) || func_148()) || func_19(8, -1)) || func_145()) || func_147()) || func_146()) || func_144()) || Global_106565.f_7682.f_919[iVar0] == 5)
						{
							return 0;
						}
						break;
					...
					case 2:
						if ((((((((((((((((((!IS_PLAYER_READY_FOR_CUTSCENE(PLAYER_ID()) || IS_ENTITY_IN_AIR(PLAYER_PED_ID())) || IS_PED_GETTING_INTO_A_VEHICLE(PLAYER_PED_ID())) || IS_PED_RAGDOLL(PLAYER_PED_ID())) || IS_PED_FALLING(PLAYER_PED_ID())) || IS_PLAYER_BEING_ARRESTED(PLAYER_ID(), 1)) || IS_PLAYER_CLIMBING(PLAYER_ID())) || IS_PED_IN_COMBAT(PLAYER_PED_ID(), 0)) || func_149()) || Global_105612) || Global_25767) || func_148()) || func_19(8, -1)) || func_145()) || func_147()) || func_146()) || func_144()) || Global_106565.f_7682.f_919[iVar0] == 5) || Global_36972 != -1)
						{
							return 0;
						}
						break;
					...
					case 3:
						if ((((((((((((IS_PED_RAGDOLL(PLAYER_PED_ID()) || IS_PED_FALLING(PLAYER_PED_ID())) || IS_PLAYER_BEING_ARRESTED(PLAYER_ID(), 1)) || IS_PED_IN_COMBAT(PLAYER_PED_ID(), 0)) || func_149()) || Global_105612) || Global_25767) || func_148()) || func_19(8, -1)) || func_147()) || func_146()) || func_144()) || Global_106565.f_7682.f_919[iVar0] == 5)
						{
							return 0;
						}
						break;
					...
					case 4:
						if (((((func_149() || GET_PLAYER_WANTED_LEVEL(PLAYER_ID()) > 0) || func_19(8, -1)) || func_144()) || func_143()) || Global_106565.f_7682.f_919[iVar0] == 5)
						{
							return 0;
						}
						break;
					...
					case 7:
						if ((((((((((((((((((IS_PED_IN_COMBAT(PLAYER_PED_ID(), 0) || !IS_PLAYER_CONTROL_ON(PLAYER_ID())) || !IS_PLAYER_READY_FOR_CUTSCENE(PLAYER_ID())) || !IS_SCREEN_FADED_IN()) || IS_ENTITY_IN_AIR(PLAYER_PED_ID())) || IS_PED_RAGDOLL(PLAYER_PED_ID())) || IS_PED_FALLING(PLAYER_PED_ID())) || IS_PLAYER_BEING_ARRESTED(PLAYER_ID(), 1)) || func_149()) || func_146()) || Global_105612) || Global_25767) || func_148()) || Global_37584) || func_19(8, -1)) || func_145()) || func_143()) || func_144()) || Global_106565.f_7682.f_919[iVar0] == 5)
						{
							return 0;
						}
						break;
					...
					case 8:
						if (((((((((((((((((((((IS_PED_IN_COMBAT(PLAYER_PED_ID(), 0) || !IS_PLAYER_CONTROL_ON(PLAYER_ID())) || !IS_PLAYER_READY_FOR_CUTSCENE(PLAYER_ID())) || !IS_SCREEN_FADED_IN()) || IS_PLAYER_WANTED_LEVEL_GREATER(PLAYER_ID(), 0)) || IS_ENTITY_IN_AIR(PLAYER_PED_ID())) || IS_PED_IN_ANY_VEHICLE(PLAYER_PED_ID(), 1)) || IS_PED_RAGDOLL(PLAYER_PED_ID())) || IS_PED_FALLING(PLAYER_PED_ID())) || IS_PED_SWIMMING(PLAYER_PED_ID())) || IS_PLAYER_BEING_ARRESTED(PLAYER_ID(), 1)) || IS_PLAYER_CLIMBING(PLAYER_ID())) || func_149()) || Global_105612) || Global_25767) || func_148()) || func_19(8, -1)) || func_145()) || func_143()) || func_147()) || func_146()) || func_144())
						{
							return 0;
						}
						break;
...
void func_157()
{
	...
	if (iLocal_47 != 0)
	{
		...
		REMOVE_RELATIONSHIP_GROUP(uLocal_61);
		_0xA5F377B175A699C5(20000);
		REMOVE_SCENARIO_BLOCKING_AREA(uLocal_65, 0);
		REMOVE_SHOCKING_EVENT(iLocal_64);
		func_107(0);
	}
	if (!IS_PED_INJURED(uLocal_52) && !IS_PED_INJURED(PLAYER_PED_ID()))
	{
		TASK_SMART_FLEE_PED(uLocal_52, PLAYER_PED_ID(), 100f, -1, 0, 0);
		SET_PED_KEEP_TASK(uLocal_52, 1);
		WAIT(0);
	}
	func_158(-1);
