...
void __EntryFunction__()
{
	...
	while (true)
	{
		...
		if (NETWORK_IS_GAME_IN_PROGRESS())
		{
			...
			func_1();
			iLocal_54++;
			if (iLocal_54 >= _NETWORK_GET_NUM_PARTICIPANTS_HOST())
			{
				iLocal_54 = 0;
			}
		}
		...
		if ((iLocal_54 % iVar0) == 0)
		{
			if (NETWORK_IS_PARTICIPANT_ACTIVE(INT_TO_PARTICIPANTINDEX(iVar0)))
			{
				...
				if (!IS_ENTITY_DEAD(uVar2, 0))
				{
					...
					{
						if (VDIST2(GET_ENTITY_COORDS(PLAYER_PED_ID(), 0), GET_ENTITY_COORDS(uVar2, 1)) <= 22500f)
						{
							if (!iLocal_56)
							{
							...
							if (!IS_BIT_SET(iLocal_55, iVar0))
							{
								SET_BIT(&iLocal_55, iVar0);
							}
						}
						else if (IS_BIT_SET(iLocal_55, iVar0))
						{
							CLEAR_BIT(&iLocal_55, iVar0);
						}
					}
					else if (IS_BIT_SET(iLocal_55, iVar0))
					{
...
void func_2()
{
	...
	{
		case 0:
			if (((func_25(PLAYER_ID(), 1) || func_24()) || !bVar3) || func_17())
			{
				iLocal_52 = 4;
			}
			else
		...
		case 3:
			if ((HAS_SCRIPT_LOADED("pb_prostitute") && _GET_NUMBER_OF_INSTANCES_OF_SCRIPT_WITH_NAME_HASH(joaat("pb_prostitute")) <= 0) && !NETWORK_IS_SCRIPT_ACTIVE("pb_prostitute", NETWORK_PLAYER_ID_TO_INT(), 1, 0))
			{
				iVar4 = func_12(iVar2);
		...
		case 5:
			if (((!func_25(PLAYER_ID(), 1) && !func_24()) && bVar3) && !func_3())
			{
				iLocal_53 = GET_GAME_TIMER() + 250;
				iLocal_52 = 0;
			}
...
int func_4()
{
	if (_GET_NUMBER_OF_INSTANCES_OF_SCRIPT_WITH_NAME_HASH(joaat("pb_prostitute")) <= 0 && !NETWORK_IS_SCRIPT_ACTIVE("pb_prostitute", NETWORK_PLAYER_ID_TO_INT(), 1, 0))
	{
		return 1;
	}
	if (iLocal_57 == 0)
...
{
	vLocal_58[NETWORK_PLAYER_ID_TO_INT() /*3*/] = iParam0;
}
...
int func_12(int iParam0)
{
	if (iParam0 == -1)
	{
		return vLocal_58[NETWORK_PLAYER_ID_TO_INT() /*3*/];
	}
		...
		if (DOES_ENTITY_EXIST(PLAYER_PED_ID()))
		{
			if (IS_PED_IN_ANY_VEHICLE(PLAYER_PED_ID(), 0))
			{
				...
				if (func_28(&uVar2))
				{
					...
					{
						if (func_12(iVar1) == GET_PED_IN_VEHICLE_SEAT(uVar3, 0, 0) || IS_PED_IN_VEHICLE(func_12(iVar1), uVar3, 1))
						{
					...
					vVar4 = { GET_ENTITY_COORDS(func_12(iVar1), 0) };
...
int func_16(var uParam0)
{
	...
	uVar1 = GET_ENTITY_SCRIPT(uParam0, &uVar0);
	if (!IS_STRING_NULL_OR_EMPTY(uVar1))
	{
		if (ARE_STRINGS_EQUAL(uVar1, "GB_VEHICLE_EXPORT"))
		{
			return 0;
		}
		if (ARE_STRINGS_EQUAL(uVar1, "BUSINESS_BATTLES_SELL"))
		{
			return 0;
		}
		if (ARE_STRINGS_EQUAL(uVar1, "BUSINESS_BATTLES"))
		{
			return 0;
		}
		if (ARE_STRINGS_EQUAL(uVar1, "GB_CASINO"))
		{
			return 0;
		}
	}
...
int func_34()
{
	...
	if (func_35() != 0)
	{
		if (_GET_NUMBER_OF_INSTANCES_OF_SCRIPT_WITH_NAME_HASH(func_35()) == 0)
		{
			return 1;
		}
	}
...
void func_49(struct<21> Param0)
{
	...
	func_50(0, -1, 0);
	NETWORK_REGISTER_PLAYER_BROADCAST_VARIABLES(&vLocal_58, 97);
	if (NETWORK_IS_GAME_IN_PROGRESS())
	{
		func_5();
	}
