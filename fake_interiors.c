...
void __EntryFunction__()
{
	...
	while (true)
	{
		...
		{
			uLocal_33 = PLAYER_PED_ID();
		}
		if (IS_ENTITY_DEAD(uLocal_33, 0))
		{
		}
		func_1();
	}
...
void func_1()
{
	...
	{
		vVar0 = { GET_ENTITY_COORDS(uLocal_33, 0) };
		vVar3 = { func_19(iLocal_29) };
		if (VDIST2(vVar3, vVar0) < IntToFloat(func_18(iLocal_29)))
		{
			bVar6 = false;
			iVar7 = 0;
			iVar7 = 0;
			while (iVar7 < func_17(iLocal_29))
			{
				if (!bVar6)
				{
					if (IS_ENTITY_IN_ANGLED_AREA(uLocal_33, func_16(iLocal_29, iVar7), func_15(iLocal_29, iVar7), func_14(iLocal_29, iVar7), 0, 1, 0))
					{
						bVar6 = true;
					}
				}
			...
			if (bVar6)
			{
				SET_RADAR_AS_INTERIOR_THIS_FRAME(GET_HASH_KEY(func_13(iLocal_29)), vVar3.x, vVar3.y, func_12(iLocal_29), func_11(iLocal_29));
				func_10(iLocal_29);
			}
		}
	}
	if (NETWORK_IS_GAME_IN_PROGRESS())
	{
		if (func_2())
		{
			if (IS_BIT_SET(uLocal_34, 1))
			{
				CLEAR_BIT(&uLocal_34, 1);
			}
			if (IS_PAUSE_MENU_ACTIVE())
			{
				if (!IS_BIT_SET(uLocal_34, 0))
				{
					SET_MINIMAP_COMPONENT(15, 1, -1);
					SET_BIT(&uLocal_34, 0);
				}
			}
			else if (IS_BIT_SET(uLocal_34, 0))
			{
				SET_MINIMAP_COMPONENT(15, 0, -1);
				CLEAR_BIT(&uLocal_34, 0);
			}
		}
		else if (!IS_BIT_SET(uLocal_34, 1))
		{
			SET_MINIMAP_COMPONENT(15, 0, -1);
			CLEAR_BIT(&uLocal_34, 0);
			SET_BIT(&uLocal_34, 1);
...
void func_21()
{
	...
	if (iLocal_28 != iLocal_29)
	{
		...
		{
			vVar0 = { GET_ENTITY_COORDS(uLocal_33, 0) };
			if (VDIST2(func_19(iLocal_28), vVar0) < VDIST2(func_19(iLocal_29), vVar0))
			{
				iLocal_29 = iLocal_28;
			}
		}
