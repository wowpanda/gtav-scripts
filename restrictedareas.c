						...
						if (TIMERA() > 125)
						{
							...
							if (func_29(iLocal_37, GET_ENTITY_COORDS(PLAYER_PED_ID(), 0)))
							{
								...
								if (func_28(iLocal_37))
								{
									...
									if (_GET_NUMBER_OF_INSTANCES_OF_SCRIPT_WITH_NAME_HASH(GET_HASH_KEY(&Local_72)) == 0)
									{
								...
								else if (!Global_25979[iLocal_37])
								{
									if (IS_PED_IN_FLYING_VEHICLE(PLAYER_PED_ID()))
									{
										...
										if (!bLocal_56)
										{
											...
											{
												if ((GET_GAME_TIMER() - uLocal_78[iLocal_70]) > 8000)
												{
													func_18();
												}
											...
											{
												uLocal_78[iLocal_70] = GET_GAME_TIMER();
											}
										}
									}
									else
									{
										...
										if (!bLocal_56)
										{
											if (func_32(GET_ENTITY_COORDS(PLAYER_PED_ID(), 0), iLocal_37, 0, 0))
											{
												if (!iLocal_37 == 4 || (GET_GAME_TIMER() - uLocal_78[iLocal_70]) > 8000)
												{
													func_18();
												}
											}
...
int func_23()
{
	...
	if (IS_PED_IN_ANY_VEHICLE(PLAYER_PED_ID(), 0))
	{
		...
		{
			if ((IS_THIS_MODEL_A_BICYCLE(GET_ENTITY_MODEL(uVar0)) || GET_ENTITY_MODEL(uVar0) == joaat("caddy")) || GET_ENTITY_MODEL(uVar0) == joaat("caddy2"))
			{
				return 1;
			}
			else
...
int func_32(vector3 vParam0, int iParam3, int iParam4, bool bParam5)
{
	vector3 vVar0[15];
	vector3 vVar46[15];
	float fVar92[15];
